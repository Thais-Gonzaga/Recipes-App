...
describe('20 - Implemente os botões de categoria para serem utilizados como filtro', () => {
  ...
  it('Verifica a cobertura de 45% da tela de receitas', () => {
    cy.task('getCoverage', getId()).its('Recipes.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('Recipes.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('Recipes.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
  });
});
...
describe('23 - Redirecione a pessoa usuária ao clicar no card para a tela de detalhes, que deve mudar a rota e conter o id da receita na URL', () => {
  ...
  it('Verifica a cobertura de 90% da Tela de Receitas', () => {
    cy.task('getCoverage', getId()).its('Recipes.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('Recipes.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('Recipes.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
