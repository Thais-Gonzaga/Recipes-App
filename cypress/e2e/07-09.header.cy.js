...
describe('8 - Redirecione a pessoa usuária para a tela de perfil ao clicar no botão de perfil', () => {
  ...
  it('Verifica a cobertura de 45% do Componente Header', () => {
    cy.task('getCoverage', getId()).its('Header.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('Header.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('Header.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
  });
...
describe('9 - Desenvolva o botão de busca que, ao ser clicado, a barra de busca deve aparecer. O mesmo serve para escondê-la', () => {
  ...
  it('Verifica a cobertura de 90% do Componente Header', () => {
    cy.task('getCoverage', getId()).its('Header.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('Header.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('Header.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
