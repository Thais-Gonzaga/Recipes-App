...
describe('Done recipes screen', () => {
  ...
  describe('46 - Desenvolva a tela de maneira que, caso a receita do card seja uma bebida, ela deve possuir: a foto da receita, o nome, se é alcoólica, a data em que a pessoa fez a receita e um botão de compartilhar', () => {
    ...
    it('Verifica a cobertura de 45% da tela de Receitas Feitas', () => {
      cy.task('getCoverage', getId()).its('DoneRecipes.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('DoneRecipes.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('DoneRecipes.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
    });
  });
  ...
  describe('49 - Redirecione para a tela de detalhes da receita caso seja clicado na foto ou no nome da receita', () => {
    ...
    it('Verifica a cobertura de 90% da tela de Receitas Feitas', () => {
      cy.task('getCoverage', getId()).its('DoneRecipes.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('DoneRecipes.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('DoneRecipes.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
    });
  });
});
